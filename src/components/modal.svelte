...
<!--		<div class="mx-auto my-36 w-[600px] bg-white rounded-md shadow-xl p-10" style="width: {width}" on:click|stopPropagation>-->
		<div class="mx-auto my-36 bg-white rounded-md shadow-xl p-10" style="width: {width}" on:click|stopPropagation>
			{#if header}
				<div class="header">
					<div class="title">
						{#if title}{title}{/if}
						...
						<div class="controls">
							{#if controlsOptions}
								<button use:clickOutside
												on:click_outside={() => { if(userDropdown) userDropdown = !userDropdown }}
