		...
		try {
			...
			if (!isEdit) {
				projectsOptions = response.data
					.filter((project) => project.queryingUserRole === 'Lider')
					.map((project) => ({
	...
	async function validate() {
		try {
			const formHasChanged = () => {
				return Object.keys(formData).some((key) => formData[key] !== loadedFormData[key]);
			};
			...
			if (isEdit) {
				...
				if (formHasChanged()) {
					...
					Object.keys(rewardObj).forEach(
						(key) => rewardObj[key] === undefined && delete rewardObj[key]
					);

					await axios
						.put(`https://luma-server.onrender.com/api/rewards/`, rewardObj)
						.then((response) => {
						...
						.catch((error) => {
							...
							if (error.response) {
								...
								if (error.response.status === 400 || error.response.status === 403) {
									if (
										error.response.data === 'El usuario no tiene permisos para editar el proyecto.'
									) {
				...
				axios
					.post('https://luma-server.onrender.com/api/rewards/create', {
					...
					.catch((error) => {
						...
						if (error.response) {
							...
							if (error.response.status === 400 || error.response.status === 403) {
								if (
									error.response.data ===
									'El usuario no tiene permisos de líder para crear recompensas.'
								) {
									showToast($t('create_reward.not_leader'), { type: 'warning', duration: 5000 });
									return;
								} else {
